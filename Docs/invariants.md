...
- **Enforcement evidence or expected validation:** Not yet implemented — this invariant governs the future readiness feature. `Docs/architecture.md` notes the schema already stores check-in fields (sleep/energy/soreness/stress) with RLS scoping consistent with "private by default," but no UI currently writes a real user-entered check-in (confirmed by a read-only reconciliation review: no call site for `saveCheckIn` exists in `app/`). Whether any future UI/copy meets the "never medical" bar must be checked against ADR-0002 before shipping. Any new check-in data written for real users must satisfy I-1 (RLS verified) and I-6 (own-data-only access) before it reaches production — check-in data is not exempt from those requirements.
...
- **Enforcement evidence or expected validation:** Currently **not met** by the existing readiness composite, per a read-only reconciliation review (2026-07-27): `src/domain/calc/readiness.ts`'s `workloadFactor` and `wellbeingFactor` substitute a neutral score (0.7) when data is absent and fold it into a single 0–100 score with no top-level low-confidence state, even though each factor does carry an explanatory `detail` sentence. `loadRecommendation.ts`'s `establish` action, by contrast, already meets this bar. Expected validation: ADR-0002 Phase A adds this state to the readiness composite, with deterministic tests, before any suggestion-audit persistence work begins.
- **Exception process:** None without an ADR update.
