...
- **We do not sell your data or share it for advertising**, and we do not use it for profiling.
  Supabase and RevenueCat process only the data needed to operate PRism, as described in §4.
...
We use no analytics vendor, advertising network, data broker, marketing platform, or support tool
that receives your training data.
...
- Data collected: **Personal info → Email address**; **Health and fitness → Fitness info** (and
  **Health info** if body measurements ship); **App activity → Other user-generated content**
  (session reflections); and **Financial info → Purchase history** for the Pro unlock.
- Data shared with third parties: Supabase and RevenueCat act as processors operating on our behalf.
  `[OWNER: confirm whether either processor relationship counts as "sharing" against Google's current
  Data Safety definitions and RevenueCat's current disclosure guidance before submitting.]`
